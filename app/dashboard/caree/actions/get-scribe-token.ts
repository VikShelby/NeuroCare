...
export async function getScribeToken(): Promise<ScribeTokenResult> {
  try {
    ...
    // Request a single-use realtime Scribe token (original working endpoint)
    ...
    if (!data?.token) {
      const detail = typeof data === "object" ? (data.detail || data.message || JSON.stringify(data)) : String(data)
      return { error: `Invalid token response${detail ? `: ${detail}` : ""}` }
    }
    console.log(data)
