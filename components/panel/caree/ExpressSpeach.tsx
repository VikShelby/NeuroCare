...
import { getScribeToken } from "../../../app/dashboard/caree/actions/get-scribe-token"
import { LanguageSelector } from "@/components/language-selector"
import { useGenMode } from "./GenModeContext"
...
const micStreamRef = useRef<MediaStream | null>(null)
  ...
  // Lightweight microphone preflight to detect missing/blocked/no-signal mics without using Web Audio
  const preflightMic = useCallback(async () => {
    try {
      ...
      if (!track) {
        ...
        return { ok: false, message: "No microphone track detected." }
      }

      // Give it a brief moment to start and then check the muted state
      await new Promise(res => setTimeout(res, 250))
      const muted = (track as any).muted === true
      const live = track.readyState === "live"

      stream.getTracks().forEach(t => t.stop())

      if (!live) {
        return { ok: false, message: "Microphone is not live. It may be in use by another app." }
      }
      if (muted) {
        return { ok: false, message: "Microphone has no signal. Check input device or permissions." }
      }
      return { ok: true }
    } catch (err: any) {
      if (err?.name === "NotAllowedError") {
        return { ok: false, message: "Please grant microphone access in your browser settings." }
      }
      if (err?.name === "NotFoundError") {
        return { ok: false, message: "No microphone found. Please connect a mic and try again." }
      }
      return { ok: false, message: "Unable to access microphone. Check device and privacy settings." }
    }
  }, [])

  // Auto-disconnect if GenMode changes while connected/connecting
  useEffect(() => {
    const isActive = connectionStateRef.current === "connecting" || connectionStateRef.current === "connected"
    if (!isActive) return

    // If mode is no longer expression-speech-support, end recording
  ...
  // Using built-in scribe state (partial and committed) instead of manual assembly
  const onPartialTranscript = useCallback((data: { text?: string }) => {
    lastTranscriptRef.current = data.text || ""
  }, [])

  const onCommittedTranscript = useCallback((data: { id?: string; text?: string }) => {
  ...
  const onCommittedTranscriptWithTimestamps = useCallback(
    (data: { id?: string; text?: string; words?: Array<any> }) => {
      // Available for consumers; no-op here except ensuring text retained
      if (data.text && data.text.length > 0) {
        finalTranscriptsRef.current = [...finalTranscriptsRef.current, data.text]
        lastTranscriptRef.current = ""
      }
  ...
  const scribeConfig = useMemo(
    ...
    [onPartialTranscript, onCommittedTranscript, onCommittedTranscriptWithTimestamps, onError]
  )
  ...
  const handleToggleRecording = useCallback(async () => {
    ...
    // DISCONNECT
    if (scribe.isConnected || connectionState === "connecting") {
    ...
    try {
      ...
      if (!mic.ok) {
        updateConnectionState("idle")
        setRecording(prev => ({ ...prev, error: mic.message || "Microphone not available" }))
        return
      }
  ...
  // Display text from scribe state (committed + partial), fallback to error
  const committedText = scribe.committedTranscripts?.map((t: any) => t.text).join(" ") || ""
  const liveText = scribe.partialTranscript || ""
  const combinedText = [committedText, liveText].filter(Boolean).join(" ")
  const displayText = recording.error || combinedText
  const hasContent = Boolean(committedText || liveText) && connectionState === "connected"
  const isPartial = Boolean(liveText && !recording.error)
    ...
    <div className="relative mx-auto flex h-full w-full max-w-4xl flex-col items-center justify-center">
      ...
      <div className="relative flex h-full w-full flex-col items-center justify-center gap-8 overflow-hidden px-8 py-12">
        ...
        <div className="relative flex min-h-[350px] w-full flex-1 items-center justify-center overflow-hidden">
          ...
          >
            {recording.error && (
              <div className="absolute -top-8 left-1/2 -translate-x-1/2 max-w-xl rounded-md border border-red-300/30 bg-red-500/10 px-3 py-2 text-sm text-red-400 shadow-sm">
                {recording.error}
              </div>
            )}
            <div
              ...
              <div className="flex flex-col items-center gap-6">
                <div className="flex flex-col items-center gap-2 text-center">
                  <h1 className="text-2xl font-semibold tracking-tight">
                    Adaptive Speech Support
                  </h1>
                  <p className="text-muted-foreground text-sm">
                    Helps you express yourself: turns your spoken words into clear, well‑structured text with gentle enhancement—supporting autistic and neurodiverse communication.
                  </p>
                </div>
